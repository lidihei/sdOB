...
class check_orbit():
    ...
    def pymcsresualt(self, trace, Teff1s=None, Teff2s=None, logg1s=None, logg2s= None, pers=None, tcs=None,
        ...
        self.jits = self.trace.posterior.jit.values.ravel() if (jits is None) else jits
        ...
        omega_rads =  np.arctan2(self.sesinws, self.secosws)
        _ind = omega_rads < 0
        omega_rads[_ind] = omega_rads[_ind] + 2*np.pi
        self.omega_rads = omega_rads
        ...
        self.R_L1 =eR_Rochelobe(1/self.qs) * self.smas
        self.R_L2 =eR_Rochelobe(self.qs) * self.smas
        self.tps = timetrans_to_timeperi(self.tcs, self.pers, self.eccs, self.omega_rads)
        g2s = self.m2s*units.Msun *constants.G /(self.R2s*units.Rsun)**2
        self.logg2s = np.log10(g2s.to('cm/s2').value) if (logg2s is None) else logg2s
        
    def trace_rv(self, trace,  pers=None, tcs=None, jits=None):
        self.trace = trace
        self.pers = trace.posterior.per.values.ravel() if (pers is None) else pers
        self.tcs =  trace.posterior.tc.values.ravel() if (tcs is None) else tcs
        self.jits = self.trace.posterior.jit.values.ravel() if (jits is None) else jits
    ...
    def trace_lc(self, trace, Teff1s=None, Teff2s=None, logg1s=None, logg2s= None, pers=None, tcs=None,
                       incls = None):
        self.trace = trace
        self.pers = trace.posterior.per.values.ravel() if (pers is None) else pers
        self.tcs =  trace.posterior.tc.values.ravel() if (tcs is None) else tcs
        ...
        self.omega_rads = omega_rads
        self.sma1s = self.smas/(1+1/self.qs)
        self.sinis = np.sin(np.deg2rad(self.incls))
        self.vsini1s = 50.592732 * self.R1s/self.pers *self.sinis
        self.L1s = 9.0093545e-16 * self.R1s**2*self.Teff1s**4
        self.L2s = 9.0093545e-16 * self.R2s**2*self.Teff2s**4
        self.k1s = 50.592732 *self.sma1s/self.pers*self.sinis
        self.sbratios = self.L2s/self.L1s
        self.k2s = self.k1s/self.qs
        self.r_1s = self.R1s/self.smas
        self.r_2s = self.R2s/self.smas
    ...
    def print_median_values(self):
        #---------------------------best point------------------------------------------------------
        _per, _tc, _tp = np.median(self.pers), np.median(self.tcs), np.median(self.tps)
        _secosw, _sesinw, _ecc, _omega_rad = np.median(self.secosws), np.median(self.sesinws), np.median(self.eccs), np.median(self.omega_rads)
        _vgamma, _jit, _k1, _k2,= np.median(self.vgammas), np.median(self.jits),np.median(self.k1s), np.median(self.k2s)
        _R1, _R2tR1, _R2, _sma =  np.median(self.R1s), np.median(self.R2tR1s), np.median(self.R2s), np.median(self.smas)
        _m1, _m2, _q = np.median(self.m1s), np.median(self.m2s), np.median(self.qs)
        _L1, _L2 = np.median(self.L1s), np.median(self.L2s)
        _incl, _r_1, _r_2,   = np.median(self.incls), np.median(self.r_1s), np.median(self.r_2s)
        _vsini1 = np.median(self.vsini1s)
        _sbratio= np.median(self.sbratios)
        print('----------------------median values----------------------------------')
        print(f'm1, m2, q = {_m1}, {_m2}, {_q} ')
        print(f'per, tc, tp  = {_per}, {_tc}, {_tp}')
        print(f'ecc, omega, secosw, sesinw, incl= {_ecc}, {_omega_rad}, {_secosw}, {_sesinw},  {_incl}')
    ...
    def ellc_lc(self, lc, tc=None, q=None, per=None, sma=None,
              ...
              foldmethod='mean', exact_grav=False):
        ...
        ldc1 = xgb_lyd(np.log10(Teff1), logg1, np.log10(0.38), 2, model_dic = model_dic)
        ldc2 = xgb_lyd(np.log10(Teff2), logg2, np.log10(0.38), 2, model_dic = model_dic)
        bins=np.arange(-0.5-bins_len/2, 0.5+bins_len/2, bins_len)
        lc_primary_fold, lc_primary_foldsmooth, fluxmedian  = bin_foldlc1(lc, bins, per, tc, method=foldmethod)
        ...
        lc_0 = ellc.lc(lc.time,t_zero=tc, q=q, period=per,
                   ...
                   shape_1=shape_1,shape_2=shape_2,exact_grav=exact_grav)
        lc_1 = ellc.lc(t_lc,t_zero=tc, q=q, period=per,
                   ...
                   f_c=secosw, f_s=sesinw,
                   shape_1=shape_1,shape_2=shape_2,exact_grav=exact_grav, heat_1=heat_1, heat_2=heat_2)
        ...
        if show:
           ...
           plt.axhline(y=0)
        return lcdens, lc_resi_bins, lc_primary_foldsmooth,lc_primary_fold
...
def check_rv(t1, rv1, rv1err, t2, rv2, rv2err,
    ...
    rv2_resi
    '''
    if tdens is None: tdens = np.arange(0,1.2*per, per/500.)
