...
import numpy as np
from speedysedfit.broadsed import vgconv, rotconv
...
def broad_tlusty(ipar, pix=0.01, R = 2000, pars=None, sps=None, wave=None, laspec_conv=False):
    ...
    if laspec_conv:
       wv_new, convsp = convolution.conv_spec(wave, flux, R_hi=300000., R_lo=R, over_sample_additional=1,
        ...
        fwhm = clight/R
        print(len(wave))
        _, convsp = vgconv(wave,flux,fwhm, ppr=None)
    return pars[ipar], convsp
...
def rotbroad_template(ipar, epsilon, vsini, wvl, edgeHandling='firstlast', pars=None, sps=None, rot_conv=True):
    '''Apply rotational broadening to a spectrum. The formulae given in Gray's "The Observation
       and Analysis of Stellar Photospheres". 
    ...
    if vsini > 0:
       if not rot_conv:
          ...
          _x, rflux=rotconv(wvl, flux, epsilon, vsini)
    else:
       rflux = flux
    return np.append(pars[ipar], vsini), rflux
